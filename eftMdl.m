function respProb = eftMdl(IntN,Wgt,data,dsetMu,setSigma,pDst)
sc          = 2;
stepSize    = deg2rad(1);
...
priorMn     = dsetMu.* sc; 
[pX_igivenS_i,mapping_fun]  = pXgivenS_EC(X,stmSpc,IntN,priorMn,priorVar,Wgt,pDst);
...
for ii = 1: numel(mu_j1)
    for jj = 1 : numel(sigma_k1)
        ...
        pMPar1(:,jj,ii) = pX_igivenS_i'*pS_igivenGaussPar_jk1'.*pGaussPar_jkgivenC1;
        pMPar0(:,jj,ii) = pX_igivenS_i'*pS_igivenGaussPar_jk0'.*pGaussPar_jkgivenC0;
    end
end
